...
4. **间歇检测（省电）**：摄像头不常开——默认每 3 分钟唤醒一次，采样 8 秒取
   ...
   不常亮。校准（首次启动或手动重新校准）会先提示你坐好，3 秒后进行 3 秒
   快速采样（约 18 帧取中位数）；没检测到人脸会在 15 秒后自动重试，基准值
   异常偏低时会提醒你重新校准。**插电时自动提速**：
   间隔缩短为 1/3（最短 30 秒，菜单信息行显示 ⚡），拔电立即恢复设定间隔。
5. **升级式判定**：某次巡检发现头部俯仰低于基准 −15°（可调）→ 切换为
   **实时跟踪**（摄像头保持开启）直到姿势恢复。跟踪期间持续低头 10 秒
   （可调）→ 系统通知 + 提示音（可选语音），每 60 秒最多一次。恢复良好坐姿
   满 30 秒后退出跟踪，进入**加强观察期**——每 1 分钟巡检一次、持续 10 分钟，
   无异常才回到常规间隔。人离开 30 秒或跟踪满 10 分钟也会自动退出跟踪。
   弯腰捡东西不会误报；头部转向（|yaw|>35°）时不判定。「实时」模式
   （摄像头常开，约 0.5–1W）仍可手动选择，主要用于调试。
6. **自动避让**：锁屏即挂起监测（`🪑🔒`，解锁后 2 秒立即巡检一次，不等
   剩余间隔）；其他应用正在使用摄像头（如视频会议）时自动跳过巡检、按原
   节奏重排，菜单信息行会显示「摄像头占用中」。
...
（实时/1/3/5 分钟）、开关菜单栏角度显示、开关语音提醒、开关「低电量模式下
仍监测」（默认关——进入低电量模式自动挂起检测，状态栏显示 `🪑🪫`，退出后
自动恢复）、切换界面语言（跟随系统 / 中文 / English）。

语音提醒使用 `AVSpeechSynthesizer`，自动选用该语言已安装的最高质量声音——
在 系统设置 → 辅助功能 → 朗读内容 → 系统声音 下载「高级」版声音效果最佳
（Siri 声音不对第三方应用开放）。
