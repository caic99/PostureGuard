...
# Version: explicit override > git tag > fallback. Keeps Info.plist in sync
# with releases (CI passes POSTUREGUARD_VERSION from the tag name).
# `|| true` keeps set -e/pipefail from aborting on tarball builds without git.
GIT_VERSION="$( { git describe --tags --always 2>/dev/null || true; } | sed 's/^v//')"
VERSION="${POSTUREGUARD_VERSION:-${GIT_VERSION:-0.0.0}}"
